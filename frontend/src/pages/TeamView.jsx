    ...
    <div className="space-y-6 sm:space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
            ...
            <div key={u._id} className="bg-surface-card p-8 rounded-xl shadow-2xl border border-[var(--color-border-subtle)] group/card relative overflow-hidden">
              <div className="flex justify-between items-start mb-8 relative z-10">
                ...
                <div className="flex gap-2 opacity-0 group-hover/card:opacity-100 transition-opacity">
                  ...
                  <button onClick={() => handleDeleteClick(u)} disabled={u._id === currentUser?.id} className="p-3 bg-[var(--color-surface-base)] rounded-xl border border-[var(--color-border-subtle)] disabled:opacity-20"><Trash2 className="w-5 h-5" /></button>
                </div>
              </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 backdrop-blur-md">
          ...
          <div className="bg-surface-card rounded-[2.5rem] shadow-2xl w-[90%] sm:w-[420px] relative z-70 border border-[var(--color-border-subtle)] flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="py-2 px-7 space-y-6 overflow-y-auto" autoComplete="off">
              ...
              {!isViewMode && (
                <button type="submit" disabled={isSubmitting} className="w-full py-3 mb-4 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest">
